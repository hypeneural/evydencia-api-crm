...
// Limite Z-API: 10MB para vídeos de status
final class SendVideoStatusAction
{
        ...
        try {
            ...
            return $this->responder
                ->error(
                    ...
                    $traceId,
                    'bad_gateway',
                    'Falha ao enviar vídeo de status via WhatsApp.',
                    502,
                    $details
                )
                ->withHeader('X-Request-Id', $traceId);
            ...
            return $this->responder
                ->internalError($response, $traceId, 'Erro interno ao enviar vídeo de status.')
                ->withHeader('X-Request-Id', $traceId);
        }
    ...
    private function validate(array $payload): array
    {
        ...
        try {
            ...
            $errors[] = [
                'field' => 'video',
                'message' => 'Vídeo deve ser uma string não vazia.',
            ];
            $videoValue = null;
        }
        ...
        if (is_string($videoValue)) {
            ...
            if (!$this->isValidVideoInput($trimmedVideo)) {
                $errors[] = [
                    'field' => 'video',
                    'message' => 'Vídeo deve ser uma URL http(s) válida ou data URI.',
                ];
            }
        }
