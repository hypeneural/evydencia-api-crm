...
final class SendStickerAction
{
    ...
    /**
     * @OA\Post(
     *     path="/v1/whatsapp/sticker",
     *     tags={"WhatsApp"},
     *     summary="Envia sticker via WhatsApp",
     *     @OA\RequestBody(required=true, @OA\JsonContent(ref="#/components/schemas/WhatsAppStickerPayload")),
    ...
    public function __invoke(Request $request, Response $response): Response
    {
        ...
        $sticker = trim((string) $payload['sticker']);
        $options = $this->extractOptions($payload);

        try {
            $result = $this->service->sendSticker($traceId, $phone, $sticker, $options);
            ...
            return $this->responder
                ->error(
                    ...
                    $traceId,
                    'bad_gateway',
                    'Falha ao enviar sticker via WhatsApp.',
                    502,
                    $details
                )
                ->withHeader('X-Request-Id', $traceId);
        } catch (RuntimeException $exception) {
            $this->logger->error('Unexpected error while sending WhatsApp sticker', [
                'trace_id' => $traceId,
                'error' => $exception->getMessage(),
            ]);

            return $this->responder
                ->internalError($response, $traceId, 'Erro interno ao enviar sticker via WhatsApp.')
                ->withHeader('X-Request-Id', $traceId);
        }
    ...
    private function validate(array $payload): array
    {
        ...
        $stickerValue = trim((string) ($payload['sticker'] ?? ''));
        try {
            v::stringType()->notEmpty()->setName('sticker')->assert($stickerValue);
            if (!$this->isValidStickerInput($stickerValue)) {
                throw new NestedValidationException();
            }
        } catch (NestedValidationException $exception) {
            $errors[] = [
                'field' => 'sticker',
                'message' => 'Sticker deve ser uma URL http(s) valida ou data URI iniciando com data:image/.',
            ];
        }
        ...
        if (array_key_exists('stickerAuthor', $payload) && $payload['stickerAuthor'] !== null) {
            try {
                v::stringType()->length(1, 255)->setName('stickerAuthor')->assert($payload['stickerAuthor']);
            } catch (NestedValidationException $exception) {
                $errors[] = [
                    'field' => 'stickerAuthor',
                    'message' => 'stickerAuthor deve possuir ate 255 caracteres.',
                ];
            }
        }
    ...
    private function extractOptions(array $payload): array
    {
        ...
        if (array_key_exists('stickerAuthor', $payload)) {
            $options['stickerAuthor'] = $this->sanitizeOptionalString($payload['stickerAuthor']);
        }
    ...
    private function isValidStickerInput(string $value): bool
    {
        if ($value === '') {
            return false;
        }
