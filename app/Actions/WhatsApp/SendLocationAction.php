...
final class SendLocationAction
{
    ...
    /**
     * @OA\Post(
     *     path="/v1/whatsapp/location",
     *     tags={"WhatsApp"},
     *     summary="Envia localizacao fixa via WhatsApp",
     *     @OA\RequestBody(required=true, @OA\JsonContent(ref="#/components/schemas/WhatsAppLocationPayload")),
    ...
    public function __invoke(Request $request, Response $response): Response
    {
        ...
        try {
            $result = $this->service->sendLocation(
                $traceId,
                $phone,
                $title,
                $address,
            ...
            return $this->responder
                ->error(
                    ...
                    $traceId,
                    'bad_gateway',
                    'Falha ao enviar localizacao via WhatsApp.',
                    502,
                    $details
                )
                ->withHeader('X-Request-Id', $traceId);
        } catch (RuntimeException $exception) {
            $this->logger->error('Unexpected error while sending WhatsApp location', [
                'trace_id' => $traceId,
                'error' => $exception->getMessage(),
            ]);

            return $this->responder
                ->internalError($response, $traceId, 'Erro interno ao enviar localizacao via WhatsApp.')
                ->withHeader('X-Request-Id', $traceId);
        }
    ...
    private function validate(array $payload): array
    {
        ...
        $stringMessages = [
            ...
            'address' => 'Endereco deve ser informado.',
        ];

        foreach (['title', 'address'] as $field) {
            $value = trim((string) ($payload[$field] ?? ''));
            try {
                v::stringType()->notEmpty()->length(1, 255)->setName($field)->assert($value);
            } catch (NestedValidationException $exception) {
                $errors[] = [
                    'field' => $field,
                    'message' => $stringMessages[$field] ?? sprintf('%s deve ser informado.', $field),
                ];
            }
        }
        ...
        try {
            v::allOf(v::numericVal(), v::between(-90, 90))->setName('latitude')->assert($latitude);
        } catch (NestedValidationException $exception) {
            $errors[] = [
                'field' => 'latitude',
                'message' => 'Latitude deve ser um valor numerico entre -90 e 90.',
            ];
        }

        $longitude = (string) ($payload['longitude'] ?? '');
        try {
            v::allOf(v::numericVal(), v::between(-180, 180))->setName('longitude')->assert($longitude);
        } catch (NestedValidationException $exception) {
            $errors[] = [
                'field' => 'longitude',
                'message' => 'Longitude deve ser um valor numerico entre -180 e 180.',
            ];
        }
