...
final class AddChatTagAction
{
    ...
    /**
     * @OA\Put(
     *     path="/v1/whatsapp/chats/{phone}/tags/{tag}/add",
     *     tags={"WhatsApp"},
     *     summary="Atribui uma etiqueta a um chat do WhatsApp",
     ...
     *     @OA\Response(response=200, description="Etiqueta atribuida", @OA\JsonContent(ref="#/components/schemas/WhatsAppGenericResponse")),
    ...
    public function __invoke(Request $request, Response $response): Response
    {
        ...
        try {
            $result = $this->service->addChatTag($traceId, $phone, $tag);
            ...
            return $this->responder
                ->error(
                    ...
                    $traceId,
                    'bad_gateway',
                    'Falha ao atribuir etiqueta no WhatsApp.',
                    502,
                    $details
                )
                ->withHeader('X-Request-Id', $traceId);
        } catch (RuntimeException $exception) {
            $this->logger->error('Unexpected error while adding WhatsApp chat tag', [
                'trace_id' => $traceId,
                'error' => $exception->getMessage(),
            ]);

            return $this->responder
                ->internalError($response, $traceId, 'Erro interno ao atribuir etiqueta no WhatsApp.')
                ->withHeader('X-Request-Id', $traceId);
        }
